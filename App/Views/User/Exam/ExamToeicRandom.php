...
// Lưu câu trả lời vào session khi form được gửi
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    foreach ($_POST as $question_id => $answer) {
        $session->set($question_id, $answer);
    }
}
...
foreach ($question_answer as $answer) :
    if ($answer['question_id'] == $value['id']) {
        // Lấy câu trả lời từ session nếu có
        $selected_answer = $session->get($value['id']);
        echo '<label class="radio-inline">';
        echo '<br><input type="radio" name="' . $value['id'] . '" value="' . $i . '" id="question.' . $value['id'] . '" onclick="markColor(this.id)" ' . ($selected_answer == $i ? 'checked' : '') . '>' . $answer['text'] . '<br><br>';
        $i++;
        echo '</label>';
    }
endforeach;
